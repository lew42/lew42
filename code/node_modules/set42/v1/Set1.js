...
var SetMfn = module.exports = Mfn.extend({
	...
	prop: function(mod, prop, value){
		...
		if (is.fn(mod["set_" + prop])) {
			...
			this.log("simple assign", prop, value);
		...
		// Mfn is one use case for having .clone w/o .set
			...
			// but, this doesn't work if you .set({ mfn: {...} }) that already hasOwn...  in that case, we want to just recursively .set
	...
	assignProp: function(mod, prop, value){
		if (mod.assign && mod.assign.mfn){
			mod.assign(prop, value);
		} else if (mod.assign && !mod.assign.mfn){
			if (mod.name !== "setMfn")
				console.warn("this pattern won't work with the old Base.assign, that doesn't support .assign(name, value).");
			mod[prop] = value;
		} else {
			mod[prop] = value;
		}
...
logger.install(SetMfn);
logger.install(SetMfn.prototype);