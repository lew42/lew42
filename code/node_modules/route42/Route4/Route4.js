...
var Route = module.exports = Mod2.Sub.extend({
	...
	push: function(){
		if (window.location.pathname === this.path)
			this.log.warn("path matches, skipping");
		else 
			this.router.history.push(this.path);
	},
	alreadyActive: function(){
		this.log.warn("already active")
	},
	activate3: function(push){
		if (this.isActive){
			...
			if (this.isActiveAncestor){
				return this.activeChild.teardown()
					.then(this.setup.bind(this)); 
			} else if (this.parent.isActive){
				return this.setup();
			} else {
				return this.parent.setup()
					.then(this.setup.bind(this));
			}
		}
		this.setup();
	...
	setup2: function(){
		if (this.isActiveNode){
			this.alreadyActive();
		} else if (this.isActiveAncestor){ // aka "has" this.activeChild
			this.activeChild.teardown(); // recursively tearsdown all activeChildren
			this.setupState();
	...
	setup: function(){
		this.router.setActiveNode(this);
	},
	setActiveNode: function(route){
		this.activeNode = route;
	},
	activeChild: function(){
		var current = this.router.activeNode;
		while (current){
	...
	teardown: function(){
		if (this.isActive){
			if (this.isActiveAncestor)
				this.activeChild.teardown();
			this.teardownState();
		} else {
			this.log.warn("not active");
		}
	},
	// assume this is called for all activeChildren before calling for self
	// assume we'll also call setupState on the parent afterwards
	...
	isActiveAncestor: function(){
		return this.router.activeNode.isDescendantOf(this);
	},
	...
	/* 
	Another option - just dynamically check these options.

	route.isActive()
		if 
		router.activeRoute.isDescendantOf(this)
			loop through activeRoute's .parents
	...
	.isActive = .isActiveNode || .isActiveAncestor

	.activeChild
	.isActiveParent = .activeChild.isActiveNode
	...
	router.isActiveNode = false;
	router.isActiveAncestor = true;
	router.activeChild = one;

	
	one.isActive = true;
	one.isActiveAncestor = true;

	two.isActive = true;
	two.isActiveAncestor = true;
	two.isActiveParent = true;

	three.isActiveNode = true;
	three.isActive = true;
	-->
	/four/five/six/
	...
	*/
	// assume that parent has been setup before calling this
	setupState: function(){
		this.router.activeNode = this;
	...
	setActiveChild: function(child){
		...
		if (this.parent){
			this.parent.isActiveParent = false;
			this.parent.
		}
	},
