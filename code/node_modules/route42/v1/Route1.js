...
var Route = Base3.extend({
	...
	config_name: function(o){
		o.part && (this.name = o.part);
	},
	/*
	* This searches children.  Maybe it should be called "matchChildren", like before...
	* this.match("whatever") --> returns true if it matches self?
	* 		hmm, returning true || {} || false makes it harder to handle.. 
	*/
	match: Method({expand: false, contain: false}, function(path){
		...
		if (firstPart in this.routes){
			...
			/*
				This is a tricky function flow: when we split, and then rejoin.  I've heard rumors about having multiple return statements.  Especially when they're nested within return statements.  I mean, you ahve to be really sure what's going on, 100% of the time... 

				I think, if you're guaranteed to hit a return statement, then you've accounted for all cases.  If there's an edge case where no return gets hit, then you've missed something...
			*/
	...
	path: Method({expand: false, contain: false, enabled: false}, function(){
		if (this.parent)
			return this.parent.path() + this.part + "/";
		else
			return this.part;
