var Mod = require("mod42/v4");
var is = require("util42").is;
var $ = require("jquery");

var copy = require("copy42")({
	arrItem: function(value){
		if (value instanceof View){
			// console.log("cloning .content view?");
			return value.clone();
		} else {
			return value;
		}
	}
}).fn;

var ViewCaptor = require("../ViewCaptor");

var View = module.exports = Mod.extend({
	name: "View8",
	tag: "div",
	captive: true,
	captor: true,
	autoRender: true,
	autoAppend: true,
	adopt: true,
	enabled: true,
	cbs: [],
	dcbs: [],
	previousCaptors: [],
	set: {
		override: function(sub, value, parent, name){
			if (value instanceof sub.constructor){
				value.autoAppend = false; // it has already been added
				parent.assign(name, value);
				return true;
			}
			return false;
		},
		value: function(value){
			this.set_content(value);
		}
	},
	instantiate: function(o){
		this.inst_el(o);
		this.content = copy(this.content);
		Mod.prototype.instantiate.apply(this, arguments);
	},
	assignedTo: function(parent, name){
		var adopted = this.adoption(parent, name);

		if (adopted){
			this.$el.remove(); // it was probably captured already
			this.addClass(name);

			if (this.autoAppend){
				parent.set_content({
					_ref: name
				});
				this.autoAppend = false;
				this.autoRender = false; // prevents autoRender immediately upon cloning, which then prevents re-render after modifications
			}
		}
	},
	inst_el: function(o){
		if (o && o.tag){
			this.tag = o.tag;
			delete o.tag;
		}

		this.$el = $("<" + this.tag + ">").addClass(this.$el.attr("class")).attr("style", this.$el.attr("style"));


		this.name.indexOf("view8") === -1 && this.name.toLowerCase() !== this.tag && this.addClass(this.name);

		// console.log(this.name, this.tag);
		// damnit - this runs before .set, when extending View
		// and, if we extend again before instantiating, we never remove the class
		/// for example:  View.extend({ "section" }) Section.extend("whatever")
		// the second Section keeps the "section" class...
		if (this.name === this.tag){
			this.removeClass(this.tag);
		}

		if (this.$el.attr("class") === "")
			this.attr("class", null);
	},
	init: function(){
		if (this.autoRender)
			this.render(); // forcing the user to call render manually is probably a good idea...
			// that way, they have to choose whether to put their code before or after render, and not forget how it works
	},
	set_content: function(value){
		if (!is.def(this.content))
			this.content = value;
		else if (is.arr(this.content)){
			if (is.arr(value))
				this.content.push.apply(this.content, value);
			else
				this.content.push(value);
		} else {
			this.content = [this.content];
			if (is.arr(value)){
				this.content.push.apply(this.content, value);
			} else {
				this.content.push(value);
			}
		}
	},
	// render() is also used to re-capture
	render: function(){
		if (this.enabled){
			// set to false to skip
			this.captive && this.getCaptured();
				// move this to after rendering?

			if (!this.hasOwnProperty("isRendered") || !this.isRendered){
				this.captureFn(this.main);
				this.behaviors();
				this.bindings();
				this.rendered();
				this.isRendered = true;
			}
		}
		return this;
	},
	main: function(){
		this.capture(this.content);
		// console.log(this.content);
	},
	// catch and add to .content[]
	keep: function(fn){},
	// catch and release:  
	catch: function(fn){},
	// override points
	behaviors: function(){},
	bindings: function(){},
	rendered: function(){},
	rerender: function(){
		this.isRendered = false;
		this.render();
	},
	capture: function(value, value2){ /// + argument loop
		if (!is.def(value)){
			return false;
		} else if (is.fn(value)){
			this.captureFn(value);
		} else if (is.obj(value)){
			if (is.fn(value.render)){
				console.log("capturing .render", value);
				this.captureFn(value.render, value);
			} else if (value._ref){
				console.log("capturing ."+value._ref);
				this.capture(this[value._ref]);
			}
		} else if (is.arr(value)){
			for (var i = 0; i < value.length; i++){
				console.log(value[i]._ref);
				this.capture(value[i]);
			}
		} else {
			this.append(value); // DOM, view
		}
	},
	captureFn: function(fn, ctx){
		var ret;
		this.captor && this.becomeCaptor();
		ret = fn.call(ctx || this, this); // allow fn to bind to something else, and then use the first arg as the reference to the view
		this.captor && this.restoreCaptor();

		if (is.def(ret) && ret !== ctx){
			console.warn("capturing function return value");
			this.capture(ret);
		}
	},
	becomeCaptor: function(){
		if (this.captor){
			this.previousCaptors.push(ViewCaptor.captor);
			ViewCaptor.captor = this;
		}
	},
	restoreCaptor: function(){
		if (this.captor){
			ViewCaptor.captor = this.previousCaptors.pop();
		}
	},
	getCaptured: function(){
		if (this.captive && ViewCaptor.captor){
			ViewCaptor.captor.add(this);
		}
	},
	// these come from within captured fns
	add: function(view){
		// this is all that really needs to happen
		// view.$el.appendTo(this.$el);
		if (view.$el)
			this.append(view);
		else if (view.el)
			this.append(view.el);
		else 
			console.warn("uh oh");
	},
	// a jQuery filler text plugin
	filler: function(quantity){
		var lorem = new Lorem();
		lorem.type = Lorem.TEXT;
		lorem.query = quantity;
		lorem.createLorem(this.$el[0]);
		return this;
	},
	activate: function(){
		if (!this.active){
			this.active = true;

			if (this.route)
				this.route.activate();

			this.execCBs.apply(this, arguments);
		}
	},
	execCBs: function(){
		for (var i = 0; i < this.cbs.length; i++){
			this.cbs[i].apply(this, arguments);
		}
	},
	then: function(cb){
		this.cbs.push(cb);
	}
}).set({
	// extend: {
	// 	instantiatePrototype: function(NewView, OldView){
	// 		// protect prototype chain
	// 		console.error("wtf?");
	// 		NewView.prototype.inst_el();
	// 	}
	// },
	x: function(){
		/*
		Can we just do something like...
		this.apply(null, arguments);
		--> null makes sure that the Constructor will do the "new"
		*/
		return new (this.bind.apply(this, [null, {
			autoRender: false
		}].concat([].slice.call(arguments))));
	},
	child: function(){
		// doesn't work - "apply is not a constructor"
		return new this.apply(null, arguments);
	},
	make: function(n){
		var args = [].slice.call(arguments);
		args.shift();
		for (var i = 0; i < n; i++){
			// console.log(args);
			this.apply(null, args);
		}
	},
	sub: function(){
		var args = [].slice.call(arguments);
		// bad - first arg is used for config stuff... 
		// should put args in front, then add the flag to the end
		return this.apply(null, [{ autoRender: false }].concat(args));
	}
});

View.extend = View.extend.mfn.clone({
	instantiatePrototype: function(NewView, OldView){
		// protect prototype chain
		NewView.prototype.inst_el();
	}
}).fn;

View.prototype.$el = $("<div>");


/*   Forward jQuery fns to view.$el   */
// this just maps view.whatever() to view.$el.whatever(), so you can view.hide(), or view.show(), etc...
// if we view.append(anotherView), we need to pass anotherView.$el
// I think a rather simple check for .$el should work... 
var aliasFnToEl = function(fn){
	return function(view){
		var ret;
		// if we .. append(view), we really need to append(view.$el)
		if (view && view.$el)
			ret = this.$el[fn].call(this.$el, view.$el);
		// otherwise, just pass all arguments along, untouched
		else 
			ret = this.$el[fn].apply(this.$el, arguments);
		
		// sometimes jQuery returns a new collection with the same items, so check the actual DOM element
		if (ret && ret[0] === this.$el[0]){
			// return the view, not the $el, for chaining
			return this;
		} else {
			return ret;
		}
	};
};

// I've been adding as needed - not an exhaustive list
[	'append', 'prepend', 'click', 'show', 'hide', 'appendTo', 'prependTo', 'addClass', 'removeClass', 'toggleClass',
	'css', 'attr', 'remove', 'empty', 'hasClass', 'html', "text", "slideDown", "slideUp", "fadeIn", "fadeOut", "animate", "slideToggle", "fadeToggle", "toggle"].forEach(function(v){
		View.prototype[v] = aliasFnToEl(v);
});



/*  Create some standard elements, put them on the View constructor, and on the prototype  */
var Element;
["p", "h1", "h2", "h3", "button", "section"].forEach(function(v){
	Element = View.extend({
		name: v,
		tag: v
	});

	View[v] = Element;

	View.prototype[v] = Element;
});

["P", "H1", "H2", "H3", "Button", "Section", "Main", "H4", 
	"H5", "H6", "Blockquote", "Pre", "Code", "Ul", "Ol", "Li", 
	"Img", "Script", "Style", "Article", "Aside", "Nav", "Header",
	"Footer", "Menu", "Input", "Em", "Strong", "Span"].forEach(function(v){
	Element = View.extend({
		name: v,
		tag: v.toLowerCase()
	});

	Element.prototype.$el.removeClass(v.toLowerCase());

	View[v] = Element;
});

var Icon = View.Icon = View.extend({
	name: "Icon",
	tag: "i",
	addClass: "icon fa fa-fw",
	enabled: false,
	set: function(value){
		// console.log("set icon", value);
		if (is.str(value))
			this.type(value);
		else if (is.bool(value))
			this.enable = value;
	},
	type: function(type){
		this.enabled = true;
		this.$el.removeClass("fa-" + this._type).addClass("fa-" + type);
		this._type = type;
		return this;
	}
});

Icon.prototype.set("circle");


var Flex = View.Flex = View.prototype.flex = View.extend({
	name: "Flex",
	addClass: "flex"
});

var Item = View.Item = View.prototype.Item = Flex.extend({
	name: "Item",
	addClass: "item flex pad-children",
	set: {
		other: function(item, value){
			item.label.set(value);
			return item;
		}
	},
	icon: Icon.x(),
	label: View.x(function(){
		this.append(this.parent.name);
	}),
	btn: Icon.x("beer"),
	content: function(){
		this.icon.render();
		this.label.render();
		this.btn.render();
	}
});

var Bar = View.Bar = View.extend({
	name: "Bar",
	addClass: "bar shade"
});

var Box = View.Box = View.extend({
	name: "Box",
	addClass: "box"
});

var BarBox = View.BarBox = View.extend({
	name: "BarBox",
	addClass: "barBox collapsed",
	bar: View.Bar.x(),
	box: View.Box.x().hide(),
	collapsed: true,
	content: function(){
		this.bar.render();
		// this.box.render();
	},
	behaviors: function(){
		this.bar.click(function(){
			this.myToggle();
		}.bind(this));
	},
	myToggle: function(){
		this.box.render().appendTo(this.$el);
		if (this.collapsed){
			this.removeClass("collapsed");
			this.collapsed = false;
			this.box.slideDown();
		} else {
			this.collapsed = true;
			this.addClass("collapsed");
			this.box.slideUp()
		}
	}
});

var getIconType = function(value){
	if (is.str(value)){
		return "quote-right";
	} else if (is.num(value)){
		return "hashtag";
	} else if (is.bool(value)){
		return value ? "check" : "times";
	} else if (is.fn(value)) {
		return "play";
	} else {
		return "circle"
	}
}

View.inspect = function(value, name){
	name = is.def(name) ? name : "noname";
	if (is.obj(value)){
		// if (value.render){
		// 	return value.render();
		// } else {
			return View.Obj({
				name: name,
				obj: value
			});
		// }
	} else if (is.arr(value)){
		return View.Arr({
			name: name,
			arr: value
		});
	} else {
		return View.Var({
			name: name,
			value: value
		})
	}
};

var Var = View.Var = Bar.extend({
	name: "Var",
	addClass: "var",
	content: function(){
		View.Icon(getIconType(this.value));
		View(this.name).addClass("name");
		View(typeof this.value).addClass("type");
		if (is.fn(this.value))
			View("&lt;fn&gt;");
		else if (is.def(this.value))
			View(this.value.toString());
		else 
			View("undefined");
	}
});

var Obj = View.Obj = BarBox.extend({
	addClass: "obj",
	bar: function(){
		View.Icon("cube");

		var name = this.parent.name;
		View(name).addClass("name");

		if (this.parent.obj.id)
			View(this.parent.obj.id).addClass("id");

		View(this.parent.obj.constructor.name).addClass("class");
		
		View("{object}")
	},
	box: function(){
		for (var i in this.parent.obj){
			View.inspect(this.parent.obj[i], i);
		}
	}
});

var Arr = View.Arr = BarBox.extend({
	addClass: "arr",
	bar: function(){
		View.Icon("align-left");

		View(this.parent.name).addClass("name");

		View("[" + this.parent.arr.length + "]");
	},
	box: function(){
		for (var i = 0; i < this.parent.arr.length; i++){
			// View.Bar(function(){
			// 	View(i);
				View.inspect(this.parent.arr[i], i);
			// }.bind(this));
		}
	}
});