...
var Set = module.exports = Mfn.extend({
		...
		methods: {
			...
			arg: function(value, name){
				if (name === "mod")
					...
					return this.log.value(value, name);
			}
		},
		method: {
			exec: false,
	...
	propWithSet: function(mod, prop, value){
		if (mod[prop].set.mfn){
			...
			mod[prop].set(value);
		} else if (mod[prop].clone) {
			mod[prop] = mod[prop].clone(value);
		} else {
			// if it's a reference, not meant to be cloned, you should find a better way to modify it, rather than via a referencer's .set()
			throw "mod should have .clone() to protect prototype";
		}
	},
